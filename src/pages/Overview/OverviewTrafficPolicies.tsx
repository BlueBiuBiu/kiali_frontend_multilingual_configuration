...
export class OverviewTrafficPolicies extends React.Component<OverviewTrafficPoliciesProps, State> {
  ...
  onAddRemoveAutoInjection = () => {
    ...
    API.updateNamespace(this.props.nsTarget, jsonPatch, this.props.nsInfo.cluster)
      ...
      .catch(error => {
        AlertUtils.addError($t('helpTip61') + this.props.nsTarget, error);
      });
  };
      ...
      this.promises
        ...
        .then(_ => {
          ...
          if (op !== 'delete') {
            ...
            AlertUtils.add(
              `${$t('TrafficPolicies')} ` + op + 'd for ' + ns + ' namespace.',
              'default',
              MessageType.SUCCESS
            );
            this.props.load();
        ...
        .catch(errorDelete => {
          if (!errorDelete.isCanceled) {
            AlertUtils.addError(`${$t('helpTip62')} `, errorDelete);
          }
        });
    } else {
      this.createTrafficPolicies(ns, duration, apsP, sdsP, op, cluster);
  ...
  createTrafficPolicies = (
    ...
    graphDataSource.on('fetchError', (errorMessage: string | null) => {
      if (errorMessage !== '') {
        errorMessage = `${$t('tip41')}: ` + errorMessage;
      } else {
        errorMessage = $t('tip42');
      }
