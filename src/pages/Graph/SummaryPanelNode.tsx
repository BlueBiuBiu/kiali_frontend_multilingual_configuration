        ...
        <ExpandableSection
          toggleText={
            nodeData.hasWorkloadEntry.length === 1
              ? `1 ${$t('Workload_Entry')}`
              : `${nodeData.hasWorkloadEntry.length} ${$t('workloadEntries')}`
          }
    ...
    return (
      <div style={{ marginTop: '0.5rem', marginBottom: '0.5rem' }}>
        ...
        {isOutOfMesh && !serverConfig.ambientEnabled && (
          <div>
            <KialiIcon.OutOfMesh />
            <span style={{ paddingLeft: '0.25rem' }}>{$t('HasMissingSidecar')}</span>
          </div>
        )}
        {isOutOfMesh && serverConfig.ambientEnabled && (
          <div>
            <KialiIcon.OutOfMesh />
            <span style={{ paddingLeft: '0.25rem' }}>{$t('OutOfMesh')}</span>
          </div>
        )}
        {isDead && (
          <div>
            ...
            <span style={{ paddingLeft: '0.25rem' }}>{$t('HasNoRunningPods')}</span>
          </div>
        )}
        {hasRequestRouting && (
          <>
