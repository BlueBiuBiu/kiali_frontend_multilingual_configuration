...
export class ClusterDropdownComponent extends React.PureComponent<ClusterDropdownProps, ClusterDropdownState> {
  ...
  private clusterButtonText() {
    if (this.state.selectedClusters.length === 0) {
      return <span>{$t('SelectClusters')}</span>;
    }
    ...
    return (
      <div className={headerStyle}>
        <span style={{ display: 'flex' }}>
          ...
          />
          {hasFilter && (
            <Tooltip key="ot_clear_cluster_filter" position="top" content={$t('ClearFilterByName')}>
  ...
  private getBody() {
    ...
    return <div className={optionStyle}>{$t('NoClustersFound')}</div>;
  }
