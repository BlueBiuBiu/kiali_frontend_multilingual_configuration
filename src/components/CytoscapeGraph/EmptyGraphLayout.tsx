    ...
    if (this.props.isLoading) {
      return (
        <EmptyState id="empty-graph-is-loading" variant={EmptyStateVariant.lg} className={emptyStateStyle}>
          <EmptyStateHeader titleText={$t('LoadingGraph')} headingLevel="h5" />
        </EmptyState>
      );
    }
    ...
    if (this.props.namespaces.length === 0) {
      return (
        <EmptyState id="empty-graph-no-namespace" variant={EmptyStateVariant.lg} className={emptyStateStyle}>
          <EmptyStateHeader titleText={$t('title7')} headingLevel="h5" />
          <EmptyStateBody>{$t('tip162')}</EmptyStateBody>
        </EmptyState>
      );
    }
    ...
    if (isGraphEmpty && !this.props.isMiniGraph) {
      return (
        <EmptyState id="empty-graph" variant={EmptyStateVariant.lg} className={emptyStateStyle}>
          <EmptyStateHeader titleText={$t('EmptyGraph')} headingLevel="h5" />
          <EmptyStateBody>
            ...
            {this.props.showIdleNodes && <> {$t('tip163')}</>}
            {!this.props.showIdleNodes && <> {$t('tip164')}</>}
          </EmptyStateBody>
          <EmptyStateFooter>
            <Button
            ...
            >
              {(this.props.showIdleNodes && <>{$t('Refresh')}</>) || <>{$t('DisplayIdleNodes')}</>}
            </Button>
          </EmptyStateFooter>
        </EmptyState>
      );
    ...
    if (isGraphEmpty && this.props.isMiniGraph) {
      return (
        <EmptyState id="empty-mini-graph" variant={EmptyStateVariant.lg} className={emptyStateStyle}>
          <EmptyStateHeader titleText={$t('EmptyGraph')} headingLevel="h5" />
          <EmptyStateBody>{$t('tip165')}</EmptyStateBody>
        </EmptyState>
      );
    }
