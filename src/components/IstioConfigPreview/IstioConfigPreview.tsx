...
export class IstioConfigPreview extends React.Component<Props, State> {
  ...
  render() {
    return (
      <Modal
        width={'75%'}
        title={this.props.title ? this.props.title : $t('PreviewTrafficPolicies')}
      ...
      >
        <Toolbar>
          ...
          >
            <ToolbarItem>
              <Tooltip content={<>{$t('CopyAllResources')}</>}>
        ...
        {this.props.disableAction && <div className={kialiStyle({ color: PFColors.Danger })}>{$t('tip232')}</div>}
      </Modal>
    );
  }
}
